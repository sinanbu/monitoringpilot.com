...
public partial class YalovaShips : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Byalovaships.Disabled = true;
            Byalovaships.Style.Add("color", "#111111");
    ...
    public List<isliste> NewShipListvipCBind(SqlConnection baglanti)
    {
        ...
        SqlCommand cmdisokunsl = new SqlCommand("SP_Isliste_NSL_vipCyalova", baglanti);
    ...
    private void yalportShipList(SqlConnection baglanti)
    {
        using (PilotdbEntities2 isliste = new PilotdbEntities2())
        {
            DLyalportShipList.DataSource = yalportShipListBind(baglanti);
            DLyalportShipList.DataBind();
        }
    }
    public List<isliste> yalportShipListBind(SqlConnection baglanti)
    {
        ...
        List<isliste> NSLyalport = new List<isliste>();

        if (dr.HasRows)
        {
            while (dr.Read())
            {
                NSLyalport.Add(new isliste()
                {
                    id = Convert.ToInt32(dr["id"].ToString()),
                    imono = Convert.ToInt32(dr["imono"].ToString()),
                    gemiadi = dr["gemiadi"].ToString(),
    ...
    public List<isliste> AnchoredShipListBind(SqlConnection baglanti)
    {
        ...
        SqlCommand cmdisokunsl = new SqlCommand("SP_Isliste_NSL_ASLyalova", baglanti);
        cmdisokunsl.CommandType = CommandType.StoredProcedure;
        SqlDataReader dr = cmdisokunsl.ExecuteReader();
    ...
    {//satır sec
        ...
        { 
            ...
            else if (uzatmakelimesi.Substring(0, 1) == "+")
            {
                ...
                td.Attributes.Add("style", "background-color:#66aaee");
            }
            ...
            Label Lbl5kop = (Label)e.Item.FindControl("Lbl5");
            string yanasmalimani = Lbl5kop.Text;
            ...
            if (yanasmalimani != "Yelkenkaya")
            {
                ...
                if (uyari != "")
                {
                    Lbl5kop.Style.Add("font-weight", "bold");
                    Lbl5kop.ToolTip = yetkili + " " + telno + " " + cepno + " " + uyari;
                }
                else
                { Lbl5kop.ToolTip = yetkili + " " + telno + " " + cepno; }
            }

            // kalkış yanaşma demir rengi kırmızı
            Label Lbl3kop = (Label)e.Item.FindControl("Lbl3");
            Label Lbl3rgkop = (Label)e.Item.FindControl("Lbl3rg");
            if (Lbl3rgkop.Text.ToLower() == "anch.")
            {
                Lbl3kop.Style.Add("color", "#ee1111");
            }

            Label Lbl5rkop = (Label)e.Item.FindControl("Lbl5r");
            Label Lbl5rgkop = (Label)e.Item.FindControl("Lbl5rg");
            if (Lbl5rgkop.Text.ToLower() == "anch.")
            {
                Lbl5kop.Style.Add("color", "#ee1111");
                Lbl5rkop.Style.Add("color", "#ee1111");
            }
    ...
    protected void DLyalportShipList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {//satır sec
        ScriptManager.RegisterStartupScript(this, GetType(), "select", "select();", true);
        SqlConnection baglanti = AnaKlas.baglan();
        ...
        if (e.Item.ItemType == ListItemType.Item | e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            Label Lbl5rihkop = (Label)e.Item.FindControl("Lbl5rih");
            if (Lbl5rihkop.Text.ToLower() == "anch.")
            {
                Lbl5ckop.Style.Add("color", "#ee1111");
                Lbl5rihkop.Style.Add("color", "#ee1111");
            }
    ...
    {//satır sec
        ...
        {  
            ...
            // kalkış yanaşma demir rengi kırmızı
            Label Lbl3kop = (Label)e.Item.FindControl("Lbl3");
            Label Lbl3rkop = (Label)e.Item.FindControl("Lbl3r");
            Label Lbl3rgkop = (Label)e.Item.FindControl("Lbl3rg");
            if (Lbl3rgkop.Text.ToLower() == "anch.")
            {
                Lbl3kop.Style.Add("color", "#ee1111");
                Lbl3rkop.Style.Add("color", "#ee1111");
            }
    ...
    protected void ImageButtonJobEdit_Click(object sender, ImageClickEventArgs e)
    {
        ...
        TBJEdrft.BorderColor = System.Drawing.Color.Gray;
        TBJEdrft.BorderWidth = 1;
        ...
        if (gemireader.HasRows)
        {
            while (gemireader.Read())
            {
                ...
                TBJEdrft.Text = gemireader["draft"].ToString();
    ...
    protected void Buttonisedit_Click(object sender, EventArgs e)
    {
        ...
        if (demiryeri != "" && yanasmalimani != "" && yanasmarihtimi != "" &&  eta != "" && bowt != "" && strnt != "" && bayrak != "" && grt != "" && tip != "" && loa != "" && tehlikeliyuk != "" && acente != "" && fatura != "" && notlar != "" && talepno != "")
        {
                ...
                string bilgi = "0";
                bilgi = AnaKlas.tpphesapla(grt, tip, loa, bowt, strnt, tehlikeliyuk, kalkisrihtimi, yanasmarihtimi);
                ...
                if (draft == "" || draft == null)
                {
                    ...
                    else if (Convert.ToInt32(grt) > 5000 && Convert.ToInt32(grt) < 45001)
                    {
                        draft = "2";
                    }
                    else if (Convert.ToInt32(grt) > 45001 && Convert.ToInt32(grt) < 75001 && tehlikeliyuk.ToLower() == "no")
                    {
                        draft = "2";
                    }
                    else if (Convert.ToInt32(grt) > 45001 && Convert.ToInt32(grt) < 500000 && (tip.ToLower() == "lpg" || tip.ToLower() == "lng" || tip.ToLower() == "tanker"))
                    {
                        draft = "3";
                    }
                    else { draft = "3"; }
    ...
    protected void Buttonpilotatac_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (nedurumdaopr == "4")
            {
                ...
                cmdistmakeconup.Parameters.AddWithValue("@etb", TBetadatetimec.Text);
                cmdistmakeconup.Parameters.AddWithValue("@etd", kayitzamani);
                cmdistmakeconup.Parameters.AddWithValue("@nedurumda", "0");
                cmdistmakeconup.Parameters.AddWithValue("@nedurumdaopr", "0");
                cmdistmakeconup.Parameters.AddWithValue("@kayitzamani", Lblpatailkkayit.Text); 
