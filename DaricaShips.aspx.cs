...
public partial class DaricaShips : System.Web.UI.Page
{
    ...
    public List<isliste> NewShipListvipCBind(SqlConnection baglanti)
    {
        ...
        SqlCommand cmdisokunsl = new SqlCommand("SP_Isliste_NSL_vipCdarica", baglanti);
        cmdisokunsl.CommandType = CommandType.StoredProcedure;
        SqlDataReader dr = cmdisokunsl.ExecuteReader();
    ...
    public List<isliste> AnchoredShipListBind(SqlConnection baglanti)
    {
        ...
        SqlCommand cmdisokunsl = new SqlCommand("SP_Isliste_NSL_ASLehs", baglanti);
        cmdisokunsl.CommandType = CommandType.StoredProcedure;
        SqlDataReader dr = cmdisokunsl.ExecuteReader();
    ...
    {   //satır sec
        ...
        if (e.Item.ItemType == ListItemType.Item | e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            // byd not rengi farklı* not rengi kırmızı
            Label Lbl3ckop = (Label)e.Item.FindControl("notlar");
            string uzatmakelimesi = Lbl3ckop.Text.ToLower() + "uzat";
            if (uzatmakelimesi.Substring(0, 3) == "byd")
            {
                Lbl3ckop.Style.Add("color", "#e100a0");
            }
            else if (uzatmakelimesi.Substring(0, 1) == "*")
            {
                Lbl3ckop.Style.Add("color", "#ee1111");
            }
            else if (uzatmakelimesi.Substring(0, 1) == "+")
            {
            ...
            if (Convert.ToDateTime(etakop.Text) > DateTime.Now.AddHours(-2))
            {
                if (Convert.ToDateTime(etakop.Text).AddHours(-2) <= DateTime.Now)
                {
                    ...
                    etasaatkop.Style.Add("font-weight", "bold"); 
                }
            }
            ...
            // demirler kısaltma
            Label Lbl5kop = (Label)e.Item.FindControl("Lbl5c");
            ...
            Label durumkop = (Label)e.Item.FindControl("Lblnedurumda");
            if (durumkop.Text.ToString() == "0")
            {
            ...
            {
                Lbl5kop.Style.Add("color", "#e100a0");
    ...
    {//satır sec
        ...
        if (e.Item.ItemType == ListItemType.Item | e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            //satır boya giriş temaslı ayır,  DataRowView drv = e.Item.DataItem as DataRowView;
            HtmlTableRow tr = (HtmlTableRow)e.Item.FindControl("Row");
            Label durumoprkop = (Label)e.Item.FindControl("Lblnedurumdaopr");
            Label durumkop = (Label)e.Item.FindControl("Lblnedurumda");


            if (durumkop.Text.ToString() == "7" && durumoprkop.Text.ToString() == "0")
            {
                tr.Attributes.Add("style", "background-color:#ffffe5");
            }
            else if (durumkop.Text.ToString() == "7" && durumoprkop.Text.ToString() != "0")
            {
                tr.Attributes.Add("style", "background-color:#DBF9F7");
            }
    ...
    protected void LBpata_Click(object sender, EventArgs e)
    {
        ...
        if (nedurumda != "7" && nedurumda != "6") // durum 67 değilse kontact açılır
        {
            ...
            if (demiryeri.ToLower() == "ordino-talep yok" || demiryeri.ToLower() == "pratika yok")
            {
                patauyaric.Text = "This ship has not a 'Ordino/Pratique', Cannot give a pilot. ";
                Buttonpilotatac.Enabled = false;
            }
        ...
        CBdeleteis.Enabled = true;
        CBdeleteis.Checked = false;
        Buttonpilotatac.Text = "Make Contacted";
        if (nedurumda == "4")
        { CBdeleteis.Enabled = false; }
    ...
    protected void Buttonpilotatac_Click(object sender, EventArgs e)
    {
        ...
        else // tikliyse
        {
            ...
            cmdistmakeconup.CommandType = CommandType.StoredProcedure;
            cmdistmakeconup.Parameters.AddWithValue("@id", secimedit);
            cmdistmakeconup.Parameters.AddWithValue("@eta", kayitzamani);
            cmdistmakeconup.Parameters.AddWithValue("@etb", ""); 
            cmdistmakeconup.Parameters.AddWithValue("@etd", "");
            cmdistmakeconup.Parameters.AddWithValue("@nedurumda", "4");
            cmdistmakeconup.Parameters.AddWithValue("@nedurumdaopr", "4");
            cmdistmakeconup.Parameters.AddWithValue("@kayitzamani", kayitzamani);
            cmdistmakeconup.ExecuteNonQuery();
            cmdistmakeconup.Dispose();
        }
...
//        ScriptManager.RegisterClientScriptBlock(this, typeof(System.Web.UI.Page), "MyJSFunction", "tamam();", true);
    ...
    protected void CBdeleteis_CheckedChanged(object sender, EventArgs e)
    {
        if (CBdeleteis.Checked==true)
        { Buttonpilotatac.Text = "Cancel Contacted"; }
        else
        { Buttonpilotatac.Text = "Make Contacted"; }
